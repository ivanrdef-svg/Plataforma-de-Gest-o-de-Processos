...
import { AlertTriangle, BadgeCheck, CircleDot, Flag, GitBranch } from "lucide-react";
import { getStepType } from "@/config/process-model";
import { diagramBounds, type BpmDiagram, type BpmNode } from "@/config/bpm-model";
...
export const BpmCanvas = forwardRef<BpmCanvasHandle, BpmCanvasProps>(function BpmCanvas(
  ...
  useImperativeHandle(ref, () => ({
    zoomBy: (factor: number) => {
      const el = containerRef.current;
      const rect = el?.getBoundingClientRect();
      zoomAt(viewRef.current.zoom * factor, (rect?.width ?? 800) / 2, (rect?.height ?? 500) / 2);
    },
    fit,
  ...
  return (
    <div
      ...
      className={cn(
        ...
        creating || connecting ? "cursor-crosshair" : panning ? "cursor-grabbing" : "cursor-grab",
        className,
      )}
      ...
      <svg className="h-full w-full" role="presentation">
        ...
        <g transform={`translate(${view.x} ${view.y}) scale(${view.zoom})`}>
          {diagram.edges.map((edge) => {
            ...
            const labelX = dependency ? (a.x + a.width / 2 + b.x + b.width / 2) / 2 : (x1 + x2) / 2;
    ...
    >
      {isEvent ? (
        <>
          ...
          />
          <foreignObject x={0} y={node.height / 2 - 12} width={node.width} height={24}>
...
function Minimap({
  ...
  return (
    ...
    >
      ...
      <p className="px-0.5 pb-0.5 text-center text-[9px] text-muted-foreground">Mini mapa</p>
    </div>
  );
}
